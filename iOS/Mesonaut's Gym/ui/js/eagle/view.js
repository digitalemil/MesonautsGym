...
View.prototype.paintQuad = function(d, teximg, w, h, r) {
	var i = 6;
	var xmin = this.min(d[i], d[i + 2], d[i + 4], d[i + 6]);
	var ymin = this.min(d[i + 1], d[i + 3], d[i + 5], d[i + 7]);
	this.ctx.translate(xmin + w2 + teximg.width / 2, h2 + ymin + teximg.height
			/ 2);
	// this.ctx.translate(xmin + w2, h2 + ymin);

	this.ctx.rotate((2 * Math.PI * r) / 360.0);
	this.ctx.drawImage(teximg, -teximg.width / 2, -teximg.height / 2, w, h);
	this.ctx.rotate(-(2 * Math.PI * r) / 360.0);
	this.ctx.translate(-(xmin + w2 + teximg.width / 2),
			-(h2 + ymin + teximg.height / 2));
	// this.ctx.translate(-(xmin + w2), -(h2 + ymin));
	...
	for (var t = 0; t < this.modell.getNumberOfThings(); t++) {
		if (this.modell.getType(t) == IMAGE) {
			...
			if (this.modell.texNameChanged(t)) {
				...
				/*
				 * if (this.modell.isTexIDSet(t)) { this.textures[tex].usage--;
   ...
   var now = (new Date()).getTime();
	...
	for (var t = 0; t < this.modell.getNumberOfThings(); t++) {
		...
		if (this.modell.getType(t) == IMAGE) {
			...
			this.paintQuad(this.modell.getData(t), teximg, this.modell
					.getImageWidth(t), this.modell.getImageHeight(t),
					this.modell.getRotation(t));
			// console.log(this.modell.getImageWidth(t)+" "+
			// this.modell.getImageHeight(t))
		} else {
			if (ft == false) {
				ft = true;
...
View.prototype.loadTexture = function(name, w, h) {
	...
	/*
	 * for (i in this.textures) { var tex= this.textures[i]; //console.log("i:
	 * "+tex.name);
	 * 
	 * if (tex.name == name && tex.width == w && tex.height == h) { tex.usage++;
	 * return i; } }
	 */
...
//	alert(x+" "+y);
	Screen.getActiveScreen().touchStop(x, y);
};
...
View.prototype.down = function(e) {
	...
	Screen.getActiveScreen().touchStart(x, y);
};
