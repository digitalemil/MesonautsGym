...
Part.prototype.getRotation = function() {
	return this.rot;
};

Part.prototype.getRx = function() {
	return this.rx;
};

Part.prototype.getRy = function() {
	return this.ry;
};

Part.prototype.getRrot = function() {
	return this.rrot;
};
...
Part.prototype.partBeginTX = function() {
	if (this.intransaction) {
		throw "Already in transaction: " + this.name;
	}
...
Part.prototype.partCommitTX = function() {
	if (!this.intransaction) {
		throw "Can not commit: Not in transaction: " + this.name;
	}
	this.intransaction = false;
};
...
Part.prototype.partRollbackTX = function() {
	if (!this.intransaction) {
		throw "Can not rollback: Not in transaction: " + this.name;
	}
	this.intransaction = false;
...
Part.prototype.myatan2 = function(y, x) {
	// Replace with yoverx
	var ret;
	var coeff_1 = (Math.PI / 4.0);
	...
	ret = y < 0.0 ? -angle : angle;
	return ret;
};

Part.prototype.distanceTo= function(p) {
//	console.log(p+" "+this+" "+(this.x-this.rx)+" "+p.name);
	return (p.x+p.rx- this.x-this.rx)*(p.x+p.rx- this.x-this.rx)+(p.y+p.ry-this.y-this.ry)*(p.y+p.ry-this.y-this.ry);
};
